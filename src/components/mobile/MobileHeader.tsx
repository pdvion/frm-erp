...
export function MobileHeader({
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center h-14 px-4 bg-theme-header border-b border-theme-header safe-area-top">
      {/* Left: Back or Menu */}
      <div className="flex items-center min-w-[44px]">
        {backHref ? (
          <Link
            href={backHref}
            className="flex items-center justify-center w-11 h-11 -ml-2 rounded-full active:bg-theme-secondary transition-colors"
            aria-label="Voltar"
          >
      ...
      <div className="flex-1 flex items-center gap-2 min-w-0">
        ...
        <h1 className="text-base font-semibold text-theme truncate">
          {title}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {showNotifications && (
          <Link
            ...
            className="relative flex items-center justify-center w-11 h-11 rounded-full active:bg-theme-secondary transition-colors"
...
export function MobileDrawerMenu({ isOpen, onClose, children, brandName = "FRM ERP" }: MobileDrawerMenuProps) {
  return (
    <>
      ...
      {/* Drawer */}
      <nav
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-theme-sidebar shadow-xl transition-transform duration-300 ease-in-out safe-area-top safe-area-bottom ${
